...
class Chatbot:
    ...
    async def ask_question(
        self, question: str, advance_search=False, document_guids: List[str] = []
    ):
        try:
            related_chunks = await self.get_related_chunks(
                question, advance_search, document_guids
            )
    ...
    async def get_related_chunks(
        self, query: str, advance_search=False, document_guids: List[str] = []
    ) -> List[str]:
        try:
            related_documents = self.vector_store.search_documents(
                query, advance_search, document_guids
            )
